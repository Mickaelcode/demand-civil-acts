...
export const createDemand = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const attachment =
      files.length <= 1
        ? false
        : files.map((fic) => fic.buffer.toString("base64"));
    if (!attachment) {
...
export const deleteDemand = async (req: Request, res: Response) => {
  try {
    let msg = "";
    let demand = await prisma.demand.findUnique({
      where: { id: Number(req.body) },
    });
    if (!demand) {
...
export const updateDemand = async (req: Request, res: Response) => {
  const files = req.files as Express.Multer.File[];
  const { idDemande, status, paid } = req.body;
  try {
    let msg = "";
    ...
    if (files) {
      ...
      html = `<h1 style="font-family:cursive;"> Your demand are succesfully accepted here are your act demand </h1>
            ...
            `;
      const act = await prisma.acte.findUnique({
        where: { numAct: demand.numActe },
      });
      if (!act) {
        msg = "act not match";
        res.status(401).json({ msg });
...
export const notificationAdmin = async (req: Request, res: Response) => {
  try {
    ...
    const data = demands.filter(
      (demand) =>
        (demand.status === "EN_ATTENTE" && demand.paid == "NO") ||
        (demand.status === "ACCEPTE" && demand.attachment.length === 3)
    );
